...
class FlowerClient(NumPyClient):
    def __init__(self, net, trainloader, valloader, local_epochs):
        ...
        self._logged_training_config = False  # Log hyperparams once per client

        # Log dataset sizes to detect tiny/odd validation shards
        try:
            logger.info(f"[Client {os.getenv('CLIENT_ID','unknown')}] train_size={len(self.trainloader.dataset)} val_size={len(self.valloader.dataset)}")
        except Exception:
            pass
    ...
    def _apply_server_control_variates(self, config, scaffold_enabled):
        ...
        try:
            ...
            # Move control variates to the model's device to avoid device mismatch
            if isinstance(server_control, dict):
                server_control = {k: (v.to(self.device) if hasattr(v, 'to') else torch.as_tensor(v, device=self.device))
    ...
    def _update_scaffold_after_training(self, scaffold_enabled, global_weights, learning_rate):
        ...
        try:
            ...
            # Ensure both models are on the same device for tensor ops
    ...
    def _prepare_metrics(self, train_loss, bytes_down, bytes_up, round_time, scaffold_delta, accuracy=None, eval_loss=None):
        ...
        if scaffold_delta is not None:
            ...
            try:
                # Move tensors to CPU for device-agnostic serialization
                if isinstance(scaffold_delta, dict):
                    scaffold_delta = {k: (v.detach().cpu() if hasattr(v, 'detach') else torch.as_tensor(v).cpu())
                                      for k, v in scaffold_delta.items()}
    ...
    def fit(self, parameters, config):
        ...
        # Log the extracted training hyperparameters once per client (round 1 diagnostic)
        if not self._logged_training_config:
            try:
                logger.info(f"[Client {os.getenv('CLIENT_ID','unknown')}] training_config={training_config}")
            except Exception:
                pass
            self._logged_training_config = True
        ...
        os.environ["LR_GAMMA"] = str(training_config["lr_gamma"])
...
def main():
    ...
    # Always evaluate on each client's mapped shard (no global test override)
    logger.info(f"[Client leaf_{args.server_id}_client_{args.client_id}] Using MAPPED SHARD for evaluation: size={len(valloader.dataset)}")
