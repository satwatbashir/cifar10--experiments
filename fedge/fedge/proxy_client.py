...
import grpc
import logging
from fedge.task import Net, set_weights, get_weights
...
class ProxyClient(NumPyClient):
    ...
    def __init__(self, server_id):
        ...
        # Initialize model (no cloud-tier evaluation)
    ...
    def evaluate(self, parameters, config) -> Tuple[float, int, dict]:
        # Do not perform cloud-tier centralized evaluation
        bytes_down = raw_bytes(parameters)
        metrics = {"accuracy": 0.0, "bytes_down_eval": bytes_down}
        results = (0.0, 0, metrics)
