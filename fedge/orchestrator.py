...
def _assert_cloud_outputs(global_round: int) -> None:
    """
    Verify cloud finished the round without requiring a centralized/global model.
    Accept either per-cluster outputs or the cloud completion signal.
    """
    round_dir_cloud = PROJECT_ROOT / "rounds" / f"round_{global_round}" / "cloud"
    clusters_ok = (round_dir_cloud / f"clusters_g{global_round}.json").exists()
    per_cluster_any = any(round_dir_cloud.glob(f"model_cluster*_g{global_round}.pkl"))
    completed = (PROJECT_ROOT / "signals" / f"cloud_round_{global_round}_completed.signal").exists()

    if not (completed or clusters_ok or per_cluster_any):
        raise RuntimeError(
            f"Cloud outputs missing for round {global_round}. "
            f"Expected a completion signal or per-cluster artifacts under {round_dir_cloud}."
        )
...
def evaluate_and_save_global_metrics(global_round: int) -> None:
    """Disabled: centralized/global evaluation removed by design."""
    logger.info(f"[Global Eval] Skipped centralized evaluation for round {global_round}")
    return
...
def run() -> None:
    ...
    try:
        for gr in range(1, GLOBAL_ROUNDS + 1):
            ...
            # Centralized global evaluation disabled (no global metrics)
