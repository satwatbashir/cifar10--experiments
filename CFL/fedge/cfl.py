...
class CFL(FedAvg):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.personalize_eval = personalize_eval
    ...
    def _try_split_cluster(self, c: int, triplets):
        # Check participation requirement based on strict_participation setting
        members = list(self.clusters[c])
        round_cids = [cid for cid, _, _ in triplets]
        
        if self.strict_participation:
            # Strict mode: require ALL cluster members to participate
            if set(round_cids) != set(members):
                return None
        else:
            # Relaxed mode: require at least 2 * min_cluster_size participants
            if len(round_cids) < 2 * self.min_cluster_size:
                return None
    ...
    def _log_split_event(self, server_round, parent_cluster, A_cids, B_cids, amax, gamma_ok, mean_norm, max_norm, child1, child2):
        ...
        path = os.path.join(self.metrics_dir, "cluster_events.csv")
    ...
    def _log_cluster_stats(self, server_round, c, mean_norm, max_norm, amax, gap, sizeA, sizeB, accept):
        ...
        os.makedirs(self.metrics_dir, exist_ok=True)
        path = os.path.join(self.metrics_dir, "cluster_metrics.csv")
        write_header = not os.path.exists(path)
        with open(path, "a", newline="") as f:
            w = csv.DictWriter(f, fieldnames=[
    ...
    def configure_evaluate(self, server_round, parameters, client_manager: ClientManager, **kwargs):
        ...
        try:
            ...
            num_avail = len(list(client_manager.all()))
        target = max(self.min_evaluate_clients, int(round(self.fraction_evaluate * num_avail)))
        sample = client_manager.sample(num_clients=target)
        ...
        for client in sample:
            ...
            nds = [t.cpu().numpy() for t in self.cluster_models[c]]
            params = ndarrays_to_parameters(nds)
            cfg = {"cid": cid, "cluster_id": c, "personalize_eval": self.personalize_eval}
    ...
    def aggregate_fit(self, server_round, results, failures, **kwargs):
        ...
        for c, triplets in by_cluster.items():
            ...
            if self.verbose:
                print(f"[CFL] Round {server_round}, Cluster {c}: "
                      f"mean_norm={mean_norm:.4f}, max_norm={max_norm:.4f}, "
                      f"ready={ready_to_consider_split}")
        ...
        # Compute total communication in MB for this round (fit only)
        total_comm_bytes = 0
        for _client, fit_res in (results or []):
            try:
                dn = int(fit_res.metrics.get("download_bytes_fit", 0))
                up = int(fit_res.metrics.get("upload_bytes_fit", 0))
                total_comm_bytes += dn + up
            except Exception:
                pass
        total_communication_MB = total_comm_bytes / (1024 * 1024)

        with open(os.path.join(self.metrics_dir, "system_metrics.csv"), "a", newline="") as f:
            w = csv.DictWriter(
                f,
                fieldnames=[
                    "round",
