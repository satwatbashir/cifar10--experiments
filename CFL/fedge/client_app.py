...
class FlowerClient(NumPyClient):
    ...
    def fit(
        ...
        inner_batches = int(len(self.trainloader) * self.local_epochs)

        # --- Prepare metrics ---------------------------------------------------
        fit_metrics = {
            # New name, keep backward-compatibility
    ...
    def evaluate(self, parameters, config) -> tuple[float, int, dict]:
        cid = config.get("cid")
        cluster_id = int(config.get("cluster_id", -1))
        personalize_eval = bool(config.get("personalize_eval", False))
        ...
        # 2) Optional personalized eval: fine‐tune one more epoch on local data
        if personalize_eval:
...
def client_fn(context):
    ...
    # Per-client deterministic seed
    set_global_seed(base_seed + int(pid))
    ...
    net = Net(in_ch=c, n_class=n_classes)
