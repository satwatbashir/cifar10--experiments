...
def _create_partitions_if_needed() -> None:
    ...
    from fedge.task import get_cifar10_dataset, CifarDataset
    import datasets
    import numpy as np
    ...
    train_cifar = get_cifar10_dataset(train=True)
    train_ds = CifarDataset(train_cifar)
    labels_np = train_ds.labels
    ...
    hf_labels = datasets.Dataset.from_dict({"label": labels_np.tolist()}).cast_column(
...
ORIG_LOCAL_STEPS: int = APP_CFG["local_steps"]
...
#  Timeout constants
# ──────────────────────────────────────────────────────────────────────────────
MAX_WAIT_SEC_LEAF: int = 300  # 5 minutes for leaf servers to complete
MAX_WAIT_SEC_CLOUD: int = 180  # 3 minutes for cloud server to complete
MAX_WAIT_SEC_PROXY: int = 120  # 2 minutes for proxy clients to complete
...
def spawn(cmd: Sequence[str], name: str, env: Dict[str, str] | None = None,
    ...
    logger.debug(f"Spawned {name}: {' '.join(cmd)} (pid={proc.pid})")
    return proc
...
SERVER_START_STAGGER = {0: 15, 1: 25, 2: 35}        # seconds
MAX_WAIT_SEC_LEAF    = SERVER_ROUNDS_PER_GLOBAL * 600 + 120   # generous buffer
MAX_WAIT_SEC_PROXY   = 600
MAX_WAIT_SEC_CLOUD   = GLOBAL_ROUNDS * SERVER_ROUNDS_PER_GLOBAL * 600 + 300
...
cloud_proc = None
leaf_server_procs: List[Tuple[int, _sp.Popen]] = []
leaf_client_procs: List[Tuple[Tuple[int, int], _sp.Popen]] = []
proxy_client_procs: List[Tuple[int, _sp.Popen]] = []
active_processes: List[Tuple[str, _sp.Popen]] = []  # General process tracking for spawn()
...
#  1) Cloud Aggregator
# ──────────────────────────────────────────────────────────────────────────────
def start_cloud_server(global_round: int) -> None:
    """
    Launch `cloud_flower.py` fresh for each global round.
    Each cloud server instance runs exactly 1 round then exits.
    """
    global cloud_proc

    # Terminate any existing cloud server
    if cloud_proc and cloud_proc.poll() is None:
        logger.info("Terminating previous cloud server")
        cloud_proc.terminate()
        cloud_proc.wait()

    logger.info(f"⛅  Spawning cloud server on port {CLOUD_PORT} (GR={global_round})")
    ...
    env = build_env(
        ...
        extra={
            ...
            "LR_INIT": str(lr_for_round(global_round)),
            "LR_DECAY": str(LR_DECAY),
            "LR_GAMMA": str(LR_GAMMA),
            "DATASET_FLAG": DATASET_FLAG,
            "CLOUD_PORT": str(CLOUD_PORT),
            "NUM_SERVERS": str(NUM_SERVERS),  # Ensure NUM_SERVERS is passed to cloud
            "SERVER_ROUNDS": str(SERVER_ROUNDS_PER_GLOBAL),
        },
    )

    cloud_script = PROJECT_ROOT / "cloud_flower.py"
    if not cloud_script.exists():
        cloud_script = PACKAGE_DIR / "cloud_flower.py"
    cmd = [sys.executable, str(cloud_script)]
    cloud_proc = spawn(cmd, name="cloud_server", env=env)
...
def launch_leaf_servers(global_round: int, prev_local_rounds: int) -> int:
    ...
    for sid in range(NUM_SERVERS):
        ...
        # Check for existing model from previous global round
        model_path = PROJECT_ROOT / "models" / f"head_{sid}.pkl"
        ...
        for cid in range(n_clients):
            cenv = build_env(
                ...
                extra={
                    ...
                    "LOCAL_STEPS": str(ORIG_LOCAL_STEPS),
                },
            )
            ccmd = [
                sys.executable,
...
# ──────────────────────────────────────────────────────────────────────────────
#  3) Proxy clients (upload server models to cloud)
# ──────────────────────────────────────────────────────────────────────────────
def start_proxy_clients(global_round: int) -> None:
    proxy_client_procs.clear()

    logger.info(f"📤 Spawning proxy clients to upload to cloud (GR {global_round})")
    for sid in range(NUM_SERVERS):
        penv = build_env(
            role="proxy_client",
            extra={
                "SERVER_ID": str(sid),
                "GLOBAL_ROUND": str(global_round),
                "DATASET_FLAG": DATASET_FLAG,  # Pass dataset flag to proxy clients
                # Tell the proxy how many server-rounds to expect:
                "SERVER_ROUNDS_PER_GLOBAL": str(SERVER_ROUNDS_PER_GLOBAL),
                "TOTAL_SERVER_ROUNDS_THIS_CLOUD": str(SERVER_ROUNDS_PER_GLOBAL),
                # Ensure Flower assigns the proxy a deterministic node_id
                "FLWR_CLIENT_NODE_ID": str(sid),
                "FLWR_NODE_ID": str(sid),
                "CLIENT_NODE_ID": str(sid),
            },
        )
        pcmd = [
            sys.executable,
        ...
        pproc = spawn(pcmd, name=f"proxy_client_{sid}", env=penv)
        proxy_client_procs.append((sid, pproc))
...
def wait_for_leaf_servers(global_round: int) -> set[int]:
    # Wait for leaf server processes
    done_servers = _wait(leaf_server_procs, "leaf server", MAX_WAIT_SEC_LEAF)
    
    # Wait for leaf server completion signals
    logger.info("⏳ Waiting for leaf server completion signals …")
    for server_id in range(NUM_SERVERS):
        completion_signal_path = (
            PROJECT_ROOT / "rounds" / f"round_{global_round}" / "leaf" /
            f"server_{server_id}" / "completion.signal"
        )
        if wait_for_file(completion_signal_path, timeout=30):
            logger.info(f"✅ Leaf server {server_id} completed successfully")
            # Keep signal file for debugging/analysis (no cleanup)
        else:
            logger.warning(f"⏰ Leaf server {server_id} completion signal timed out")
    
    return done_servers



def wait_for_global_completion(gr: int) -> bool:
    # Wait for cloud completion signal in signals folder (matches cloud_flower.py)
    completion_signal_path = PROJECT_ROOT / "signals" / "cloud_completed.signal"
    logger.info(f"⏳ Waiting for cloud completion signal: {completion_signal_path}")
    
    ok = wait_for_file(completion_signal_path, timeout=MAX_WAIT_SEC_CLOUD)
    if ok:
        logger.info("✅ Cloud server completed successfully")
        # Keep signal file for debugging/analysis (no cleanup)
    else:
        logger.error(f"⏰ Cloud server completion timed out")
    return ok
################################################################################
#  end of chunk 2  ─────────────────────────────────────────────────────────────
################################################################################
...
def start_proxy_clients(global_round: int) -> None:
    ...
    for server_id in range(NUM_SERVERS):
        ...
        logger.info(f"   ✅ Started proxy client {server_id} (node_id set via get_properties method)")
...
def wait_for_leaf_servers(global_round: int) -> None:
    """
    Wait for all leaf servers to complete their local training rounds.
    """
    logger.info(f"⏳ Waiting for {NUM_SERVERS} leaf servers to complete...")
    
    for server_id in range(NUM_SERVERS):
        completion_signal = PROJECT_ROOT / "signals" / f"round_{global_round}" / f"server_{server_id}_completion.signal"
        
        if wait_for_file(completion_signal, timeout=MAX_WAIT_SEC_LEAF):
            logger.info(f"   ✅ Leaf server {server_id} completed")
        else:
            logger.warning(f"   ⚠️ Leaf server {server_id} did not complete within {MAX_WAIT_SEC_LEAF}s")


def wait_for_global_completion(global_round: int) -> bool:
    """
    Wait for cloud server to complete aggregation and evaluation.
    """
    completion_signal = PROJECT_ROOT / "signals" / "cloud_completed.signal"
    
    if wait_for_file(completion_signal, timeout=MAX_WAIT_SEC_CLOUD):
        logger.info("✅ Cloud server completed successfully")
        return True
    else:
        logger.error(f"❌ Cloud server did not complete within {MAX_WAIT_SEC_CLOUD}s")
        return False
...
def evaluate_and_save_global_metrics(global_round: int) -> None:
    ...
    try:
        ...
        models_dir = PROJECT_ROOT / "models"
        candidate_paths = []
        # Prefer an explicitly saved global model if present
...
def run() -> None:
    ...
    logger.info(f"📋 Run summary created: {summary_file}")
    ...
    try:
        for gr in range(1, GLOBAL_ROUNDS + 1):
            ...
            wait_for_leaf_servers(gr)

            # ── Step 3: Aggregate server models across leaf servers (FedAvg) ──
            if NUM_SERVERS > 1:
                aggregate_server_models(gr)
                
            # ── Step 4: Evaluate global model and save metrics ──
            logger.info("📊 Step 3: Evaluating global model performance")
