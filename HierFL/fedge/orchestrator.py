...
from fedge.partitioning import hier_dirichlet_indices, write_partitions  # noqa: E402
...
ALPHA_SERVER = hier.get("alpha_server", 0.7)
...
if not parts_json.exists():
    logger.info("Creating hierarchical Dirichlet partitions …")

    # Build labels arrays for train and test exactly matching order used in load_data
    ...
    combined = {"train": mapping_train, "test": mapping_test}
    write_partitions(parts_json, combined)
    logger.info(f"Wrote {parts_json} with hierarchical partitions (train/test)")
...
def launch_leaf_servers(global_round: int, start_round: int) -> int:
    ...
    if global_round > 0:
        # Use the PREVIOUS global round (zero-indexed) for the file path
        cand = get_global_model_path(global_round - 1)
        if cand.exists():
