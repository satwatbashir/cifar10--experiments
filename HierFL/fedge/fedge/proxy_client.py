...
class ProxyClient(NumPyClient):
    def __init__(self, server_id):
        ...
        # Path to model for this global round (no fallback)
        ...
        with open(model_path, "rb") as f:
            ...
            # Require exact tuple format (ndarrays, total_examples)
            assert isinstance(loaded_data, tuple) and len(loaded_data) == 2, "Model pickle must be (ndarrays, total_examples)"
        ...
        assert "test" in combined, "PARTITIONS_JSON must have 'test' mapping"
        server_map_test = combined["test"][str(server_id)]  # {"0": [...], "1": [...], ...}
        indices_test = [idx for lst in server_map_test.values() for idx in lst]
        # Build a validation loader over the union of this server's client test shards
        _, self.valloader, _ = load_data("cifar10", 0, 1, indices_test=indices_test)
