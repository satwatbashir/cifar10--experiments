"""fedge: A Flower / PyTorch app (SCAFFOLD server mirroring FedProx behavior)."""
...
from fedge.task import Net, get_weights, load_data, set_weights, test, set_global_seed, DATA_FLAGS
...
# Centralized convergence tracker (mirror FedProx)
...
def _evaluate_and_log_central_impl(dataset_flag: str, round_num: int, parameters, config, metrics_dir: str = "metrics", seed: int = 0):
    ...
    trainloader, testloader, num_classes = load_data(
        ...
        num_partitions=1,
        seed=seed,
        alpha=config.get("dirichlet_alpha", 0.5) if isinstance(config, dict) else 0.5,
    )
...
def server_fn(context: Context):
    ...
    # Read from config (mirror FedProx)
    num_rounds = int(context.run_config.get("num-server-rounds", 150))
    fraction_fit = float(context.run_config.get("fraction-fit", 1.0))
    min_available_clients = int(context.run_config.get("min_available_clients", 10))
    ...
    # Build initial parameters via shape inference
    ...
    sample, _ = next(iter(trainloader))
    _, in_ch, H, W = sample.shape
    parameters = ndarrays_to_parameters(get_weights(Net(in_ch=in_ch, img_h=H, img_w=W, n_class=num_classes)))

    def eval_fn(round_num, parameters, config):
        # Centralized eval; write to per-seed directory and ensure seeded data
        local_cfg = {"dirichlet_alpha": dirichlet_alpha}
        return _evaluate_and_log_central_impl(dataset_flag, round_num, parameters, local_cfg, metrics_dir=metrics_dir, seed=seed)

    # Aggregation callbacks with per-seed CSV routing (mirror FedProx schemas)
    def aggregate_and_log_seeded(metrics_list: List[Tuple[int, Metrics]]) -> Metrics:
        ...
        # 1) Basic distributed metrics
        n_total = sum(n for n, _ in metrics_list)
        accs   = [m.get("test_accuracy", 0.0) for _, m in metrics_list]
        losses = [m.get("test_loss", 0.0) for _, m in metrics_list]
        ...
        # Per-client values for plotting/analysis
        ...
        # Simple comm time estimate at 10 Mbps
        ...
        result.update({
            ...
            # Communication volume metrics (MB)
        ...
        # Append to CSV in per-seed directory with fixed field order
        csv_path = os.path.join(metrics_dir, "distributed_metrics.csv")
        write_header = not os.path.exists(csv_path)
        # Build a deterministic row with all expected columns
        fixed_fieldnames = [
            "round",
            "avg_accuracy","avg_loss","accuracy_std","loss_std",
            "acc_ci95_lo","acc_ci95_hi","loss_ci95_lo","loss_ci95_hi",
            "avg_comp_time_sec","total_comp_time_sec","std_comp_time_sec",
            "avg_upload_MB","total_upload_MB","avg_download_MB","total_download_MB",
            "total_communication_MB",
            "avg_wall_clock_sec","total_wall_clock_sec","std_wall_clock_sec",
            "avg_comm_time_sec","total_comm_time_sec",
            # per-client accuracy (1..10)
            "client_1_accuracy","client_2_accuracy","client_3_accuracy","client_4_accuracy","client_5_accuracy",
            "client_6_accuracy","client_7_accuracy","client_8_accuracy","client_9_accuracy","client_10_accuracy",
            # per-client loss (1..10)
            "client_1_loss","client_2_loss","client_3_loss","client_4_loss","client_5_loss",
            "client_6_loss","client_7_loss","client_8_loss","client_9_loss","client_10_loss",
        ]
        write_row = {k: result.get(k, "") for k in fixed_fieldnames}
        write_row["round"] = round_num
        # Ensure per-client slots are filled/padded
        for i in range(1, 11):
            ak = f"client_{i}_accuracy"
            lk = f"client_{i}_loss"
            if ak not in write_row:
                write_row[ak] = ""
            if lk not in write_row:
                write_row[lk] = ""
    ...
    def aggregate_fit_metrics_seeded(metrics_list: List[Tuple[int, Metrics]]) -> Metrics:
        ...
        fixed_fieldnames = [
            "round",
            "avg_train_loss","std_train_loss","min_train_loss","max_train_loss",
            "avg_train_accuracy","std_train_accuracy","min_train_accuracy","max_train_accuracy",
            "avg_comp_time_sec","std_comp_time_sec","total_comp_time_sec",
            "avg_upload_MB","total_upload_MB","avg_download_MB","total_download_MB",
            "avg_inner_batches","total_train_samples",
        ]
        write_row = {k: result.get(k, "") for k in fixed_fieldnames}
        write_row["round"] = round_num
        with open(path, "a", newline="") as f:
