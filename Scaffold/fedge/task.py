# fedge/task.py — CIFAR-10 ONLY (SCAFFOLD/FedProx parity)
...
def set_global_seed(seed: int) -> None:
    ...
    # Efficiency defaults: fast but non-deterministic CuDNN
...
# ───────────────────────── Model (FedProx LeNet for CIFAR-10) ─────────────────────────
class Net(nn.Module):
...
def load_data(
    ...
    Both train and test sets are partitioned using the same alpha and seed,
    mirroring FedProx behavior. Each client evaluates on its OWN test shard.
    """
    ...
    # 2) Build labels-only datasets for partitioner
    ...
    # 3) Deterministic partitioning per run
